...
class MVPGeneticAlgorithm(GeneticAlgorithm):
    ...
    def init_population(self):
        ...
        for _ in range(self.pop_size):
            ...
            for i in range(1, len(individual)-1):
                if individual[i] == 11: # skip the other car 
                    continue
    ...
    def crossover(self, parent1, parent2):
        ...
        C1 = [-1 for _ in parent2] # changed here
        C2 = [-1 for _ in parent1] # changed here
    ...
    def calc_fitness(self, individual, show_log=True):
        ...
        for i_r, j in enumerate(reversed(individual)):
            ...
            if j == 0 or j == -1: # Nothing to do with the first point
                continue
            if j > len(self.custs): # For car points
                ...
                if individual[i-1] == -1:
                    # find the next one (or there can be no more)
                    nomore = False
                    prev = i-1
                    while individual[prev] == -1:
                        if individual[prev] >= 11 or prev == 0:
                            nomore = True
                            break
                        prev-=1

                    if not nomore:
                        distance = self.dist_matrix[individual[prev] - 1, individual[i] - 1]
                    else:
                        distance = 0
                else:
                ...
                total_cost += weight * distance
                total_distance += distance
                total_demand += individual_cars [cur_car_id -1 - len(self.custs)]['demand']
        ...
        if not invalid:
            fitness = (total_demand / total_cost) * 100 if total_cost > 0 else 0
            if show_log:
...
# make a dum car that doesnt hold anything 
#car_types.append({
#            "id": len(car_types),
#            "capacity": 9999,
#           "cost": 0
#        })
num_cars = len(car_types)
